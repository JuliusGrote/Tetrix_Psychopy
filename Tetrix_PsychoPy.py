...
def comp_wm_load_speed(total_trials, trial_nr):
    ...
    Unfortunately it is less complicated to define the function here compared to importing it from another module.
    '''
    ...
    if trial_nr == 0: 
        ...
        if Comp_wm_load and not Comp_speed:
        ...
        elif Comp_speed and not Comp_wm_load:
            speed_low = 'low_speed'
        ...
        elif Comp_speed and Comp_wm_load:
...
def skip_part(part):    
    ...
    if part == "pretrial" and not game.pretrial_rounds or part == "main_trials" and not N_repeats:
...
def should_skip(duration):
    '''
    create a helper function that returns True if the the duration of the a condition is set to 0 or None   
    '''
    
    return duration == 0 or duration is None
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp.setPriority('targeted_duration', 19)
    thisExp.setPriority('key_logging.keys', 11)
    thisExp.setPriority('key_logging.rt', 10)
    thisExp.setPriority('key_logging.duration', 9)
    ...
    thisExp.setPriority('participant', -14)
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_logging') is None:
        # initialise key_logging
        key_logging = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_logging',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Set skip flags for all conditions
    skip_play_Tetris = should_skip(Play_Tetris_duration)
    skip_watch_Tetris = should_skip(Watch_Tetris_duration)
    skip_motor_control = should_skip(Motor_control_duration)
    skip_fixation_cross = should_skip(Fixation_cross_duration)
    
    # set main_trial repeats to 0 if its disabled in config
    if skip_part("main_trials"):
        n_repeats = 0
    
    # or set it to 1 if (meaning PsychoPy goes once through the "main_trials.csv" list) N_repeats is not None in config
    else:
        n_repeats = 1
    ...
    # --- Initialize components for Routine "wait_1s_play_Tetris" ---
    fix_main = visual.TextStim(win=win, name='fix_main',
        text='+',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    # --- Initialize components for Routine "condition_ended_play" ---
    condtion_ended_text_1 = visual.TextStim(win=win, name='condtion_ended_text_1',
    ...
    press_button = visual.ImageStim(
        ...
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-3.0)
    key_logging = keyboard.Keyboard(deviceName='key_logging')
    
    # --- Initialize components for Routine "condition_ended_control" ---
    ...
    if not skip_part("main_trials"):  
        if not skip_play_Tetris:
    ...
    while continueRoutine:
        ...
        # ensures that processes are opened correctly in all conditions
        if is_window_open("pretrial_Tetris"):
            continueRoutine = not skip_part("main_trials")
            
        if (is_window_open("play_Tetris") and Watch_Tetris_duration is None) or is_window_open("watch_Tetris"):
            print("should continue")
           
            continueRoutine = not skip_part("pretrial") and skip_part("main_trials")
    ...
    print("---------------------------\nProcesses:")
    if is_window_open("pretrial_Tetris"):
        print(f"pretrial_Tetris loaded and paused (state: {game.toggle_pretrial.value})")
    if is_window_open("play_Tetris"):
        print(f"play_Tetris loaded and paused (state: {game.toggle_play.value})")
    if is_window_open("watch_Tetris"):
        print(f"watch_Tetris loaded and paused (state: {game.toggle_watch.value})")
    print("---------------------------")
    thisExp.nextEntry()
    ...
    continueRoutine = continueRoutine and not (skip_part("pretrial"))
    # keep track of which components have finished
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_play_Tetris or skip_part("main_trials"))
    # keep track of which components have finished
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_watch_Tetris or skip_part("main_trials"))
    # keep track of which components have finished
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_part("main_trials"))
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_part("pretrial") or skip_part("main_trials") or Exclude_explanations)
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (Exclude_explanations)
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_part("pretrial") or Exclude_explanations or not game.pretrial_staircase)
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (Exclude_explanations or skip_play_Tetris and skip_part("pretrial"))
    ...
    while continueRoutine:
        ...
        if explain_controls.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
    ...
    # if this routine is skipped the "Begin and End Routine" tabs are still executed
    if not skip_part("pretrial"):
        # get back to Psychopy window
        Get_on_top("PsychoPy")
        
        # adds the time stemp, achieved level and score to the data file
        ...
        # Toggle pretrial to pause to trigger save_recording in subprocess
        game.toggle_pretrial.value = not game.toggle_pretrial.value
        # Wait briefly for the subprocess to save the file
        core.wait(0.5)
    
        # Read and save simple replay data (logs) to detailed CSV
        if os.path.exists("replay_data.json"):
            # Append to the main replay_detailed.csv
            replay_filename = f"Data/{expInfo['participant']}_{expName}_gameplay_recording.csv"
            fieldnames = ['participant', 'condition', 'trial_n', 'block', 'action', 'time', 'abs_time', 'score', 'level', 'position_x', 'position_y', 'initial_grid']
            ...
            try:
                ...
                initial_grid = str(replay_data.get('initial_state', {}).get('grid', [])).replace(',', ';')
                ...
                with open(replay_filename, 'a', newline='') as csvfile:
                    ...
                    if 'moves' in replay_data:
                        for i, move in enumerate(replay_data['moves']):
                            row = {
                                'participant': expInfo['participant'],
                                'condition': move.get('condition', 'pretrial'),
                                'trial_n': move.get('game_over_count', 0),
                                'block': move.get('block', ''),
                                'action': move.get('action', ''),
                                'time': move.get('time', ''),
                                'abs_time': move.get('abs_time', ''),
                                'score': move.get('score', 0),
                                'level': move.get('level', 0),
                                'position_x': move.get('position_x', ''),
                                'position_y': move.get('position_y', ''),
                                'initial_grid': initial_grid if i == 0 else ''
                            }
                            writer.writerow(row)
            except Exception as e:
                print(f"Error saving pretrial detailed data: {e}")
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_part("main_trials") or skip_part("pretrial"))
    # Run 'Begin Routine' code from code_pause_n_sec
    if not (skip_part("main_trials") or skip_part("pretrial")):
        # creates coutdown
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (Exclude_explanations or skip_part("main_trials"))
    ...
    while continueRoutine:
        ...
        if controller_example_1.status == NOT_STARTED and not skip_play_Tetris:
            # keep track of start time/frame for later
        ...
        if watch_example_1.status == NOT_STARTED and not skip_watch_Tetris:
            # keep track of start time/frame for later
        ...
        if motorcontrol_example_1.status == NOT_STARTED and not skip_motor_control:
            # keep track of start time/frame for later
        ...
        if baseline_example_1.status == NOT_STARTED and not skip_fixation_cross:
            # keep track of start time/frame for later
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_play_Tetris or Exclude_explanations or skip_part("main_trials"))
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_play_Tetris or Exclude_explanations or skip_part("main_trials") or not Comp_wm_load)
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_play_Tetris or Exclude_explanations or skip_part("main_trials") or not Comp_speed)
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_motor_control or Exclude_explanations or skip_part("main_trials"))
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_watch_Tetris or Exclude_explanations or skip_part("main_trials"))
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (skip_fixation_cross or Exclude_explanations or skip_part("main_trials"))
    ...
    # update component parameters for each repeat
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (any([skip_play_Tetris, skip_watch_Tetris, skip_motor_control, skip_fixation_cross]) or Exclude_explanations or skip_part("main_trials"))
    ...
    if not skip_part("main_trials"):
        # adds the first trigger with special tag
    ...
    if skip_part("pretrial") and not skip_part("main_trials"):
    ...
    for thisMain_trial in main_trials:
        ...
        # update component parameters for each repeat
        # skip this Routine if its 'Skip if' condition is True
        continueRoutine = continueRoutine and not (skip_play_Tetris)
        ...
        # --- Prepare to start Routine "wait_1s_play_Tetris" ---
        continueRoutine = True
        # update component parameters for each repeat
        # skip this Routine if its 'Skip if' condition is True
        continueRoutine = continueRoutine and not (skip_play_Tetris)
        # keep track of which components have finished
        wait_1s_play_TetrisComponents = [fix_main]
        for thisComponent in wait_1s_play_TetrisComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "wait_1s_play_Tetris" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            # get current time
            t = routineTimer.getTime()
            ...
            # if fix_main is starting this frame...
            if fix_main.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                ...
                # update status
                fix_main.status = STARTED
                fix_main.setAutoDraw(True)
            
            # if fix_main is active this frame...
            if fix_main.status == STARTED:
                # update params
                pass
            
            # if fix_main is stopping this frame...
            if fix_main.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fix_main.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
                    fix_main.tStop = t  # not accounting for scr refresh
                    fix_main.tStopRefresh = tThisFlipGlobal  # on global time
                    fix_main.frameNStop = frameN  # exact frame index
                    # update status
                    fix_main.status = FINISHED
        ...
        # --- Ending Routine "wait_1s_play_Tetris" ---
        for thisComponent in wait_1s_play_TetrisComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        ...
        if Replay_movements and Watch_Tetris_duration:
            game.score.value = start_score
            game.level.value = start_level
        # the Routine "tetris_condition" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "condition_ended_play" ---
        continueRoutine = True
        # update component parameters for each repeat
        # skip this Routine if its 'Skip if' condition is True
        continueRoutine = continueRoutine and not (skip_play_Tetris)
        # keep track of which components have finished
        condition_ended_playComponents = [condtion_ended_text_1]
        for thisComponent in condition_ended_playComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "condition_ended_play" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            # get current time
            t = routineTimer.getTime()
            ...
            # *condtion_ended_text_1* updates
            
            # if condtion_ended_text_1 is starting this frame...
            if condtion_ended_text_1.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                condtion_ended_text_1.frameNStart = frameN  # exact frame index
                condtion_ended_text_1.tStart = t  # local t and not account for scr refresh
                condtion_ended_text_1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(condtion_ended_text_1, 'tStartRefresh')  # time at next scr refresh
                # update status
                condtion_ended_text_1.status = STARTED
                condtion_ended_text_1.setAutoDraw(True)
            
            # if condtion_ended_text_1 is active this frame...
            if condtion_ended_text_1.status == STARTED:
                # update params
                condtion_ended_text_1.setOpacity(None, log=False)
            
            # if condtion_ended_text_1 is stopping this frame...
            if condtion_ended_text_1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > condtion_ended_text_1.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
                    condtion_ended_text_1.tStop = t  # not accounting for scr refresh
                    condtion_ended_text_1.tStopRefresh = tThisFlipGlobal  # on global time
                    condtion_ended_text_1.frameNStop = frameN  # exact frame index
                    # update status
                    condtion_ended_text_1.status = FINISHED
                    condtion_ended_text_1.setAutoDraw(False)
        ...
        # --- Ending Routine "condition_ended_play" ---
        for thisComponent in condition_ended_playComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        ...
        if not skip_part("main_trials") and not skip_play_Tetris:
            ...
            if os.path.exists("replay_data.json"):
                ...
                try:
                    with open(replay_filename, 'a', newline='') as csvfile:
                        ...
                        # Log Moves
                        for i, move in enumerate(replay_data.get('moves', [])):
                                writer.writerow({
                                'participant': expInfo['participant'],
        ...
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from execute_condition
        # set Tetris to foreground
        ...
        motor_replay_times = []
        motor_replay_index = 0
        
        # Log motor flash timing
        motor_flash_times = []
        motor_flash_starts = []
        motor_flash_intervals = []
        motor_flash_active = False
        motor_flash_current_start = None
        motor_flash_last_start = None
        
        if control_condition == "motor_control" and Show_motor_rhythm and Replay_indication_motor:
            motor_replay_times = calculate_motor_replay_times(Motor_control_mode,
                                                              game.accelerate_type,
                                                              targeted_duration,
        ...
        key_logging.keys = []
        key_logging.rt = []
        _key_logging_allKeys = []
        # keep track of which components have finished
        random_control_conditionComponents = [press_shape, duration_and_fix, press_button, key_logging]
        ...
        while continueRoutine:
            ...
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # Run 'Each Frame' code from execute_condition
            ...
            if control_condition == "motor_control" and Show_motor_rhythm == True:
                ...
                # Use replay times if available
                if len(motor_replay_times) > 0 and Replay_indication_motor:
                    current_rel_time = t 
                    
                    # Advance index
                    while motor_replay_index < len(motor_replay_times) and current_rel_time > motor_replay_times[motor_replay_index]['end_time']:
                         motor_replay_index += 1
                    
                    if motor_replay_index < len(motor_replay_times):
                        event_info = motor_replay_times[motor_replay_index]
                        start_t = event_info['start_time']
                        end_t = event_info['end_time']
                        if current_rel_time >= start_t and current_rel_time < end_t:
                            is_active = True
                
                else: 
                    # other wise use fixed rhythm
                    press_rhythm = core.getTime()
                    
                    period = (game.speed.value/1000 * 2.5)
                    dur = max((game.speed.value/1000), Min_display_duration)
                  
                    min_period = dur + Min_gap
                    if period < min_period:
                        period = min_period
                    
                    if press_rhythm % period < dur:
                        is_active = True
                
                target_op = 1 if is_active else 0
            
                # log flash timing transitions
                if is_active and not motor_flash_active:
                    motor_flash_active = True
                    motor_flash_current_start = t
                    motor_flash_starts.append(t)
                    if motor_flash_last_start is not None:
                        motor_flash_intervals.append(t - motor_flash_last_start)
                    motor_flash_last_start = t
                elif (not is_active) and motor_flash_active:
                    motor_flash_active = False
                    if motor_flash_current_start is not None:
                        motor_flash_times.append({'start_time': motor_flash_current_start, 'end_time': t, 'duration': t - motor_flash_current_start})
                    motor_flash_current_start = None
            
                if Motor_symbol == "shape":
                    press_shape.setOpacity(target_op)
                elif Motor_symbol == "button":
                    press_button.setOpacity(target_op)
            ...
            # if key_logging is starting this frame...
            if key_logging.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                key_logging.frameNStart = frameN  # exact frame index
                key_logging.tStart = t  # local t and not account for scr refresh
                key_logging.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_logging, 'tStartRefresh')  # time at next scr refresh
                # update status
                key_logging.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_logging.clock.reset)  # t=0 on next screen flip
            
            # if key_logging is stopping this frame...
            if key_logging.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_logging.tStartRefresh + targeted_duration-frameTolerance:
                    # keep track of stop time/frame for later
                    key_logging.tStop = t  # not accounting for scr refresh
                    key_logging.tStopRefresh = tThisFlipGlobal  # on global time
                    key_logging.frameNStop = frameN  # exact frame index
                    # update status
                    key_logging.status = FINISHED
                    key_logging.status = FINISHED
            if key_logging.status == STARTED and not waitOnFlip:
                theseKeys = key_logging.getKeys(keyList=[pygame.key.name(Pygame_key_1), pygame.key.name(Pygame_key_2), pygame.key.name(Pygame_key_3), pygame.key.name(Pygame_key_4)], ignoreKeys=["escape"], waitRelease=False)
                _key_logging_allKeys.extend(theseKeys)
                if len(_key_logging_allKeys):
                    key_logging.keys = [key.name for key in _key_logging_allKeys]  # storing all keys
                    key_logging.rt = [key.rt for key in _key_logging_allKeys]
                    key_logging.duration = [key.duration for key in _key_logging_allKeys]
        ...
        for thisComponent in random_control_conditionComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # Run 'End Routine' code from execute_condition
        # finalize motor flash log
        if control_condition == "motor_control" and Show_motor_rhythm == True:
            end_rel_time = globalClock.getTime(format='float') - condition_started
            if motor_flash_active and motor_flash_current_start is not None:
                motor_flash_times.append({'start_time': motor_flash_current_start, 'end_time': end_rel_time, 'duration': end_rel_time - motor_flash_current_start})
                motor_flash_active = False
                motor_flash_current_start = None
        ...
        # check responses
        if key_logging.keys in ['', [], None]:  # No response was made
            key_logging.keys = None
        main_trials.addData('key_logging.keys',key_logging.keys)
        if key_logging.keys != None:  # we had a response
            main_trials.addData('key_logging.rt', key_logging.rt)
            main_trials.addData('key_logging.duration', key_logging.duration)
        # the Routine "random_control_condition" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "condition_ended_control" ---
        continueRoutine = True
        # update component parameters for each repeat
        # keep track of which components have finished
        condition_ended_controlComponents = [condition_ended_text]
        for thisComponent in condition_ended_controlComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "condition_ended_control" ---
        routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            # get current time
            t = routineTimer.getTime()
        ...
        # --- Ending Routine "condition_ended_control" ---
        for thisComponent in condition_ended_controlComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        ...
        # log motor-control flash timing with other condition data
        if control_condition == "motor_control" and Show_motor_rhythm == True:
            thisExp.addData('motor_flash_times', motor_flash_times)
            thisExp.addData('motor_flash_starts', motor_flash_starts)
            thisExp.addData('motor_flash_intervals', motor_flash_intervals)
    ...
    # Remove replay data file if it exists
    try:
        if os.path.exists("replay_data.json"):
            os.remove("replay_data.json")
    except Exception as _e:
        print(f"Warning: could not remove replay_data.json: {_e}")
    
    # Cleanup unwanted thisExp data entries 
    _drop_targets = Drop_columns
    
    def _should_drop(name):
        if name in _drop_targets:
            return True
    ...
    # 1) Remove keys from entries and current entry
    for key in list(thisExp.thisEntry.keys()):
        if _should_drop(key):
            thisExp.thisEntry.pop(key, None)
            _removed_keys.add(key)
    
    # 2) Remove keys from extraInfo
    for key in list(thisExp.extraInfo.keys()):
        if _should_drop(key):
            thisExp.extraInfo.pop(key, None)
            _removed_keys.add(key)
    
    # 3) Remove keys from dataNames/columnPriority (used by addData/saveAsWideText)
    thisExp.dataNames = [n for n in thisExp.dataNames if not _should_drop(n)]
    
    for key in list(thisExp.columnPriority.keys()):
        if _should_drop(key):
            thisExp.columnPriority.pop(key, None)
            _removed_keys.add(key)
    
    # 4) Filter loop param names collected by _getAllParamNames
    _orig_getAllParamNames = thisExp._getAllParamNames
    def _filtered_getAllParamNames():
        names = _orig_getAllParamNames()
        return [n for n in names if not _should_drop(n)]
    thisExp._getAllParamNames = _filtered_getAllParamNames
    
    _removed_list = sorted(_removed_keys)
    _remaining_targets = [t for t in _drop_targets if all(not k.endswith('.' + t) and k != t for k in _removed_keys)]
    print(f"Drop targets: {_drop_targets}")
    print(f"Removed keys ({len(_removed_list)}): {_removed_list}")
    if _remaining_targets:
        print(f"Targets not found in data: {_remaining_targets}")
    
    print("Cleanup of columns complete (entries, dataNames, extraInfo, priorities).")
