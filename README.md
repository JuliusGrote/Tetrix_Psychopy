...
3. motor_control: Control condition to account for basic motor activity. Subjects are instructed to press the available buttons alternately (if enabled in [`config_paradigm_psychopy.txt`](config_paradigm_psychopy.txt): a set visual press rhythm based on the current "game.speed" can be seen on the display).
...
*   **Data Logging**: A detailed CSV file `{sub}_Tetrix_PsychoPy_replay_detailed.csv` is generated in the [`Data`](Data) folder. This file contains a precise log of every action ('left', 'right', 'rotate', 'down', 'gravity', 'spawn', 'reset') with its absolute timestamp, block type, score, and level.
...
- For **analysis purposes**, the file contains the following data:
    ...
    - `game.speed` : Speed of the falling Blocks, similar to "game.level" but correlated linearly to game difficulty (for more info see [`config_tetris_game.txt`](./PyGame_Tetris_Code/config_tetris_game.txt)).\
\
**"Pre-Trial" specific:**
...
\
**If you make use of the `Jnd_regression` from [`config_tetris_game.txt`](./PyGame_Tetris_Code/config_tetris_game.txt), you will get additional parameters:**
...
\
**If you use either of the `Comp_wm_load` and/or `Comp_speed` from [`config_paradigm_psychopy.txt`](config_paradigm_psychopy.txt), you will get additional parameters:**
